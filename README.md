...
<h2>23.02.20 - HTML - </h2>
  ...
  <p>shite-space는 말줄임으로 사용하며 필수적으로 3개를 작성을 해줘야 하며 글자의 길이가 길어서 ...으로 처리가 가능한 기능이다. 또한 이 기능을 활용하기 위해서는<br>
     이기능을 활용한 선택자 틀에는 넓이 값을 정해줘여한다.</p>
  <p>글자 사이에 '|'는 면으로 지정을 해줘서 다양한 디자인을 할 수 있게 해주는 것이 좋다.<br>면으로 해서 넓이와 높이값 모양 등을 지정 해주고 위치를 설정해서 그거에 맞는 부모 
      선택자에 수정을 해줘야한다.</p>
<hr>
